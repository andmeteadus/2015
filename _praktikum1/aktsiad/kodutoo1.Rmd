...
Näpunäide: Abiks on funktsioonid `arrange()`, `select()` ja `head()`.
...
* Arvuta iga sektori kohta
  ...
  Näpunäide 2: Summade ja keskmiste arvutamisel ära võta arvesse puuduvaid väärtusi (puuduvate väärtuse välja jätmiseks on `sum` ja `mean` funktsioonidel argument `na.rm`).
