...
Kui soovime testida kõiki 106 biomarkeri olulisust surma ennustamisel, puutume kokku mitmese testimise probleemiga. Vaata selle kohta [koomiksit "Significant"](https://xkcd.com/882/) ning vajadusel selgitavat videot [Multiple Testing](https://www.youtube.com/watch?v=IDaHKAjxu_s) (vaata esimest 11 minutit). 
...
(**1 punkt**) Eelmises ülesandes nägime, et mitmed biomarkerid on omavahel tugevalt korreleeritud. Niisiis võib Bonferroni korrektsioon osutuda praegu liiga rangeks. Alternatiivselt võiksime leida, kui palju on meie andmestikus mittekorreleeritud tunnuseid, ning teha nende arvu järgi Bonferroni korrektsiooni. Selleks, et leida andmestiku nn "efektiivne dimensionaalsus", kasuta PCA-d. 
...
(**1 boonuspunkt**) Muuda eelnevat funktsiooni nii, et iga biomarkeri kordajale arvutad ka 95\% usaldusintervalli. Võid kasutada normaaljaotusel põhinevat lähendit ning arvutada selle `kordaja_hinnang +- 1.96 * SE`, kus SE on `summary(model)` väljundis toodud `Std. Error`. Funktsioon peaks nüüd lisaks tagastama listi, mille elementide seas oleks ka `lower` ja `upper`. 
