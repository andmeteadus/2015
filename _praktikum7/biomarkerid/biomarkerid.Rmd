...
<!-- f. Kui mõni on oluline, siis vali üks ning interpreteeri, mis suunalise seosega on tegu. St kas suremise tõenäosust suurendab kõrgem või madalam skoor? -->
...
### Ülesanne 4 (3 punkti) - p-väärtuse piiri paikapanek

Kui soovime testida kõiki 106 biomarkeri olulisust surma ennustamisel, puutume kokku mitmese testimise probleemiga. Vaata selle kohta [koomiksit "Significant"](https://xkcd.com/882/) ning selgitavat videot [Multiple Testing](https://www.youtube.com/watch?v=IDaHKAjxu_s) (vaata esimest 10 minutit). 
...
(**2 punkti**) Eelmises ülesandes nägime, et mitmed biomarkerid on omavahel tugevalt korreleeritud. Niisiis võib Bonferroni korrektsioon osutuda praegu liiga rangeks. Alternatiivselt võiksime leida, kui palju on meie andmestikus mittekorreleeritud tunnuseid, ning teha nende arvu järgi Bonferroni korrektsiooni. Selleks, et leida andmestiku nn "efektiivne dimensionaalsus", kasuta PCA-d. 
...
### Ülesanne 5 (3 punkti + 2 boonuspunkti) - Oluliste biomarkeri tuvastamine

Kirjuta funktsioon, mis võimaldaks ette antud mudeli kujule (näiteks `s5 ~ sugu + vanusegrupp`) lisada ükshaaval kõik biomarkerid, näiteks 

* `s5 ~ sugu + vanusegrupp + biomarker_1`
* `s5 ~ sugu + vanusegrupp + biomarker_2`

jne, et leida kõigist biomarkeritest üles kõige olulisem. 

Funktsioon võiks välja näha umbes selline:

```{r}
estimate_significance = function(formula, biomarkers, data){
  
  # sinu kood
  
  # võiksid tagastada iga biomarkeri nime, selle p-väärtuse ning kordaja suuruse
  return(list("biomarkers" = ..., "p.values" = ..., "coefs" = ...))
}
```

Juhised:

* Argument `formula` võiks olla esialgse mudeli kuju, näiteks `"s5 ~ vanusegrupp"` 
* Argument `biomarkers` võiks sisaldada kõiki biomarkereid puudutavaid veerunimesid. Näiteks esimese biomarkeri korral oleks uuendatud valem `paste("s5 ~ vanusegrupp", biomarkers[1], sep=" + ")`. 
* Argument `data` on andmestiku nimi.
* Mudeli saab sobitada `model = glm(updated_formula, family=binomial, data=data)`. 
* Mudeli p-väärtuse ja kordaja saad eraldada maatriksist `coef(summary(model))`. 

(**1 boonuspunkt**) Muuda eelnevat funktsiooni nii, et iga biomarkeri kordajale arvutad ka 95\% usaldusintervalli. Võid kasutada normaaljaotusel põhinevat lähendit ning arvutada selle `1.96 * SE`, kus SE on `summary(model)` väljundis toodud `Std. Error`. Funktsioon peaks nüüd lisaks tagastama listi, mille elementide seas oleks ka `lower` ja `upper`. 

Kontrolli, kas funktsioon töötab. 

a. Millise mudeliga alustad, st kas lisad sugu, vanusegrupp, suits, hyp esialgsesse mudelisse?
b. Millist p-väärtuse piiri kasutad? 
c. Millised biomarkerid osutuvad olulisteks? 

(**1 boonuspunkt**) Visualiseeri saadud tulemust. (Kui arvutasid eelnevalt usaldusintervallid, on üks variant visualiseerida neid.) Aga näiteks võid teha umbes sellise joonise p-väärtustest log-skaalal:

![](example_plot_biomarkers.png)

### Ülesanne 6 (2 punkti + 2 boonuspunkti) - Forward selection

Artiklis on kirjeldatud mudeli koostamist järgnevalt: 

> For biomarker discovery in the Estonian Biobank cohort, a multivariate model was derived in a forward stepwise fashion (Figure 2). First, the biomarker leading to the smallest p-value in the Cox model adjusted for age and sex only was included as a predictor. Subsequently, the biomarker leading to the smallest p-value in the multivariate model adjusted for age, sex, and the first biomarker was included in the prediction model. The process was repeated until no additional biomarkers were significant at the Bonferroni-corrected threshold of p<0.0005, accounting for testing of 106 candidate biomarkers.

Eelmises ülesandes leidsid kõige olulisema p-väärtusega biomarkeri. Jätka nüüd *forward selection*-iga:

* Lisa leitud biomarker mudelisse ning lähtu nüüd sellest mudelist. 
* Kasuta eelmises ülesandes koostatud funktsiooni ning leia nüüd järgmine biomarker, mis mudelisse lisada. 
...
(**2 boonuspunkti**) Automatiseeri eelnevalt kirjeldatud forward selection. Kirjuta funktsioon, mis lisaks mudelisse biomarkereid senikaua, kuni p-väärtus on allpool ette antud piiri. 

### Ülesanne 7 (1 punkti) - surma tõenäosuse prognoosimine

Eelmise ülesande tulemusena on sul nüüd olemas lõplik mudel, mis võtab arvesse kõik, mis on oluline surma tõenäosuse prognoosimiseks. 

Kirjuta nüüd funktsioon, mis võtaks argumendiks andmestiku vajalike tunnustega, ning prognoosiks iga andmestikus oleva inimese kohta tema tõenäosust surra 5 aasta jooksul. 
...
Kirjuta nüüd vastava funktsioon

```{r}
death_probability = function(data){
  # sinu kood
}
```

### Ülesanne 8 (1 punkti) - prognooside täpsus
