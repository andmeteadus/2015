...
```{r}
plot_digit = function(digit, add=FALSE, x_id=NA, y_id=NA, transparency=FALSE, ...){
  if(sum(is.na(x_id)) > 0){
    x_id = seq(0, 1, length=28)
  }
  if(sum(is.na(y_id)) > 0){
    y_id = seq(0, 1, length=28)
  }
...
for(i in 1:5){
  ...
  plot_digit(nullid[i, ], add=TRUE, x_id, y_id, transparency = T)
}
```
